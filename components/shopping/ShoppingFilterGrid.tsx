...
import { categories, shoppingDestinations } from "@/data/shopping";
import ShoppingCard from "@/components/shopping/ShoppingCard";

export default function ShoppingFilterGrid() {
  const [active, setActive] = useState<string>("All");

  const filtered =
    active === "All"
      ? shoppingDestinations
      : shoppingDestinations.filter((d) => d.category === active);
  ...
  return (
    <div>
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((d) => (
          <ShoppingCard key={d.slug} destination={d} />
        ))}
      </div>
    </div>
  );
